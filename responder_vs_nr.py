...
# Perform the analysis between T8 treatment responders and T8 treatment non-responders
significant_treatment_comparison = perform_t_tests(t8_treatment_responders, t8_treatment_nonresponders)

# Perform the analysis between T8 control responders and T8 control non-responders
significant_control_comparison = perform_t_tests(t8_control_responders, t8_control_nonresponders)

# Add response information to the results
significant_treatment_comparison['Response'] = 'Treatment'
significant_control_comparison['Response'] = 'Control'

# Sort by corrected p-value
significant_treatment_comparison = significant_treatment_comparison.sort_values('corrected_p_value')
significant_control_comparison = significant_control_comparison.sort_values('corrected_p_value')

# Save the results to a CSV file
output_dir = './'
significant_treatment_comparison.to_csv(f'{output_dir}significant_treatment_comparison_t8.csv', index=True)
significant_control_comparison.to_csv(f'{output_dir}significant_control_comparison_t8.csv', index=True)
